...
import {
    ArrowLeft, MessageCircle, Users, BookOpen, Calendar, Clock, Target, BarChart3,
    Square, Share2, Trophy, Zap, MessageSquare,
    Feather, CloudOff, UserCheck, Heart,
...
const StrengthenSpiritGoalWizard = () => {
    ...
    // Form Data States (Tailored for Spiritual Goals)
    ...
    const renderStep1 = () => (
        <div className="space-y-6 max-w-2xl w-full mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Practice Type */}
                <div>
                    <label htmlFor="practice-type" className="text-gray-700 font-medium block mb-2">Primary Practice</label>
                    <select
                        id="practice-type"
                    ...
                    >
                        ...
                        <option>Reading Sacred Texts</option>
                        <option>Daily Reflection</option>
                        <option>Service/Volunteer Work</option>
                    </select>
                </div>

                {/* Duration/Quantity Input */}
                <div>
                    <label htmlFor="duration-input" className="text-gray-700 font-medium block mb-2">Duration (Minutes) / Quantity</label>
                    <input
                        id="duration-input"
                        type="number"
            ...
            <div className="grid grid-cols-2 gap-4">
                {/* Frequency */}
                <div>
                    <label htmlFor="frequency" className="text-gray-700 font-medium block mb-2">Frequency</label>
                    <select
                        id="frequency"
                ...
                {/* Target Date Input */}
                <div>
                     <label htmlFor="target-date" className="text-gray-700 font-medium block mb-2">Commit until</label>
                     <input
                        id="target-date"
                        type="date"
    ...
    const renderStep2 = () => (
        <div className="space-y-6 max-w-3xl w-full mx-auto">
            <div>
                <label className="block text-gray-700 font-medium mb-2">What spiritual/mental state will this goal create?</label>
                <input type="text" placeholder="Peaceful, Focused, Compassionate, Grounded..." className="w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:ring-blue-500 text-gray-700" value={formData.outcomeFeeling} onChange={(e) => updateForm('outcomeFeeling', e.target.value)} />
            </div>
            <div>
                <label className="block text-gray-700 font-medium mb-2">How will this change your behavior in challenging situations?</label>
                <input type="text" placeholder="Respond with patience, Handle stress calmly, Forgive easily..." className="w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:ring-blue-500 text-gray-700" value={formData.outcomeAction} onChange={(e) => updateForm('outcomeAction', e.target.value)} />
            </div>
        </div>
    );

    const renderStep3 = () => {
        const options = ["Lack of time/busy schedule", "Mental distractions/noise", "Self-doubt or lack of belief", "Inconsistent routine", "Negative social influences", "Physical restlessness", "Not knowing where to start", "Other"];
        return (
            <div className="space-y-4 max-w-2xl w-full mx-auto">
                <p className="text-gray-700 mb-4 font-medium">What common blocks might interfere with your spiritual practice?</p>
    ...
    const renderStep4 = () => {
        const options = [
            { id: 'time_log', label: 'Time tracking (e.g., app screenshot)', icon: Clock, color: 'text-blue-600' },
            { id: 'journal_entry', label: 'Daily Journal Entry Summary', icon: BookOpen, color: 'text-purple-600' },
            { id: 'reflection_note', label: 'One sentence of reflective wisdom', icon: MessageCircle, color: 'text-green-600' },
            { id: 'buddy_check', label: 'Spiritual Buddy Check-ins', icon: Users, color: 'text-pink-600' },
            { id: 'mentoring', label: 'Share with a Mentor/Leader', icon: UserCheck, color: 'text-cyan-600' },
            { id: 'other', label: 'Other', icon: Square, color: 'text-gray-400' },
        ];
        return (
            <div className="space-y-4 max-w-2xl w-full mx-auto">
                 <p className="text-gray-700 mb-4 font-medium">How will you prove your consistency? <span className="text-gray-400 text-sm font-normal">(Choose one or more methods)</span></p>
                 <div className="space-y-3">
                    {options.map((opt) => (
    ...
    const renderStep5 = () => (
        <div className="space-y-6 max-w-2xl w-full mx-auto">
            <div>
                <label className="block text-gray-700 font-medium mb-2">What is the central emotion you will feel when you achieve this consistency?</label>
                <select value={formData.visFeel} onChange={(e) => updateForm('visFeel', e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option value="">Select emotion</option><option>Deep Calm</option><option>Unshakeable Peace</option><option>Boundless Love</option><option>Clarity</option><option>Joy</option>
                </select>
            </div>
             <div>
                <label className="block text-gray-700 font-medium mb-2">What simple milestone will you celebrate with?</label>
                <select value={formData.visAchieve} onChange={(e) => updateForm('visAchieve', e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option value="">Select celebration</option><option>A relaxing retreat</option><option>Buy a new meditation cushion</option><option>Send a gift to someone I care about</option><option>Take a day of silent reflection</option>
                </select>
            </div>
            <div>
                <label className="block text-gray-700 font-medium mb-2">Write a message of gratitude to your future, more mindful self</label>
                <textarea rows="4" placeholder="Dear peaceful me..." className="w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 text-gray-700 resize-none" value={formData.visMessage} onChange={(e) => updateForm('visMessage', e.target.value)} />
            </div>
        </div>
    );
    ...
    const renderStep6 = () => (
        <div className="space-y-8 max-w-2xl w-full mx-auto">
            <div className="space-y-4">
                <p className="text-gray-700">Choose a Referee - someone who supports your journey or who can check your shared reflections!</p>
                <div className="flex flex-col gap-2">
                     ...
                     <select value={formData.refereeType} onChange={(e) => updateForm('refereeType', e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full sm:w-1/2 p-2.5">
                        <option>On your Honor</option><option>Spiritual Mentor/Leader</option><option>Close Trusted Friend</option><option>Meditation Group Leader</option>
                    </select>
                </div>
            </div>
            {formData.refereeType === 'On your Honor' ? (
                <p className="text-gray-600 leading-relaxed p-4 bg-blue-50 rounded-xl border border-blue-100">
                    You have picked the honor system. You promise to be honest about your practice and reflection consistency.
                </p>
            ) : (
        ...
        <div className="space-y-8 max-w-2xl w-full mx-auto">
            <div className="flex items-center justify-between border-b border-gray-100 pb-6">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-gray-500 max-w-xs">Let goalHyke spiritual communities automatically become your Supporter</p>
                </div>
    ...
    const getStepHeader = () => {
        switch(currentStep) {
            case 1: return { icon: Target, title: "Define Your Practice" };
            case 2: return { icon: Feather, title: "Set Your Why" };
            case 3: return { icon: CloudOff, title: "Identify your challenges" };
            case 4: return { icon: BookOpen, title: "Choose your accountability tool" };
            case 5: return { icon: Zap, title: "Visualize success" };
            case 6: return { icon: UserCheck, title: "Referee" };
                ...
                <main className="p-6 md:p-10 flex-grow">
                    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                        ...
                        <div className="text-center mb-8 px-2">
                            ...
                            <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-900">Strengthen Your Spirit</h1>
                            <p className="text-sm sm:text-base text-gray-600 max-w-xl mx-auto">
                                Define your spiritual growth and commit to consistent practice.
                            </p>
                        </div>
...
export default StrengthenSpiritGoalWizard;