...
const USER_MOCK_DATA = { avatar: "/avatar-2.jpeg" };

// Removed inline Sidebar/AppHeader/Footer definitions; using shared components.
...
const ExercisePage = () => {
    ...
    // Modal States
    const [modalState, setModalState] = useState(null); // 'commit', 'token', 'success', or null
    ...
    const renderStep1 = () => (
        <div className="space-y-6 max-w-2xl w-full mx-auto">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4">
                <label className="text-gray-700 font-medium flex-1">How many days per week would you like to exercise?</label>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4">
                <label className="text-gray-700 font-medium flex-1">How long do you plan to work out each session?</label>
             ...
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4">
                <label className="text-gray-700 font-medium flex-1">What type of exercise will you focus on?</label>
                <select value={formData.type} onChange={(e) => updateForm('type', e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full md:w-40 p-2.5">
                    <option>Cardio</option><option>Strength</option><option>Yoga</option>
                </select>
            </div>
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4 pt-2">
                ...
                <span className="text-gray-900 font-semibold w-full md:w-40 px-2.5">Tuesday</span>
            </div>
        </div>
    );
    ...
    const renderStep2 = () => (
        <div className="space-y-6 max-w-3xl w-full mx-auto">
            <div>
                <label className="block text-gray-700 font-medium mb-2">What specific outcome do you want from this goal?</label>
                <input type="text" placeholder="Lose 10kg, gain muscle..." className="w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:ring-blue-500 text-gray-700" value={formData.outcome} onChange={(e) => updateForm('outcome', e.target.value)} />
            </div>
            <div>
                <label className="block text-gray-700 font-medium mb-2">How will this goal improve your life?</label>
                <input type="text" placeholder="More confidence..." className="w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:ring-blue-500 text-gray-700" value={formData.improve1} onChange={(e) => updateForm('improve1', e.target.value)} />
            </div>
            <div>
                <label className="block text-gray-700 font-medium mb-2">How will this goal improve your life?</label>
                <input type="text" placeholder="Better health..." className="w-full p-3 border border-gray-200 rounded-lg bg-gray-50 focus:ring-blue-500 text-gray-700" value={formData.improve2} onChange={(e) => updateForm('improve2', e.target.value)} />
            </div>
        </div>
    );

    const renderStep3 = () => {
        const options = ["Lack of time", "Lack of motivation", "Feeling tired or low energy", "Too many distractions", "No proper workout plan", "Lack of support or accountability", "Unhealthy eating habits", "Other"];
        return (
            <div className="space-y-4 max-w-2xl w-full mx-auto">
                <p className="text-gray-700 mb-4 font-medium">What challenges might stop you from achieving your goal?</p>
    ...
    const renderStep4 = () => {
        const options = [
            ...
            { id: 'partner', label: 'Accountability partner', icon: UsersIcon, color: 'text-blue-500' },
            ...
            { id: 'ai', label: 'AI coach (CrushIT)', icon: Sparkles, color: 'text-purple-600' },
    ...
    const renderStep5 = () => (
        <div className="space-y-6 max-w-2xl w-full mx-auto">
            <div>
                <label className="block text-gray-700 font-medium mb-2">How will you feel when you reach your goal?</label>
                <select value={formData.visFeel} onChange={(e) => updateForm('visFeel', e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option value="">Select word</option><option>Proud</option><option>Confident</option><option>Energized</option><option>Strong</option><option>Happy</option>
                </select>
            </div>
             <div>
                <label className="block text-gray-700 font-medium mb-2">What&apos;s the first thing you'll do when you achieve this goal?</label>
                <select value={formData.visAchieve} onChange={(e) => updateForm('visAchieve', e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    <option value="">Select word</option><option>Buy new clothes</option><option>Celebrate with friends</option><option>Take a progress photo</option>
                </select>
            </div>
            <div>
        ...
        <div className="space-y-8 max-w-2xl w-full mx-auto">
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                     ...
                     <select value={formData.refereeType} onChange={(e) => updateForm('refereeType', e.target.value)} className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full sm:w-1/2 p-2.5">
                        <option>On your Honor</option><option>Individual referee</option>
                    </select>
                </div>
            </div>
            {formData.refereeType === 'On your Honor' ? (
                <p className="text-gray-600 leading-relaxed p-4 bg-blue-50 rounded-xl border border-blue-100">
                    You have picked the honor system to verify your progress. It is up to you to be honest and fair towards yourself and your Supporters.
                </p>
            ) : (
        ...
        <div className="space-y-8 max-w-2xl w-full mx-auto">
            <div className="flex items-center justify-between border-b border-gray-100 pb-6">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-gray-500 max-w-xs">Let goalHyke members automatically become your Supporter</p>
                </div>
            ...
            <div className="space-y-4">
                <h3 className="text-gray-700 font-medium">Get support from your friends and family!</h3>
                ...
                <main className="p-6 md:p-10 flex-grow">
                    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                        ...
                        <div className="text-center mb-8 px-2">
                            ...
                            <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-900">Exercise Regularly</h1>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
                    ...
                    {modalState === 'commit' && (
                        <div className="bg-white rounded-3xl shadow-2xl p-8 sm:p-12 max-w-md w-full text-center transform transition-all scale-100">
                            ...
                            <div className="flex flex-col sm:flex-row justify-center gap-4">
                                <button onClick={() => setModalState('success')} className="px-8 py-3 bg-[#7C3AED] text-white rounded-full font-medium hover:bg-purple-700 transition w-full sm:w-auto">
                                    Yes, commit
                                </button>
                    ...
                    {modalState === 'token' && (
                        <div className="bg-white rounded-3xl shadow-2xl p-8 sm:p-12 max-w-md w-full text-center">
                            ...
                            <div className="flex flex-col sm:flex-row justify-center gap-4">
                                <button className="px-8 py-3 bg-[#7C3AED] text-white rounded-full font-medium hover:bg-purple-700 transition w-full sm:w-auto">
                                    Get token
                                </button>
                                <button onClick={() => setModalState(null)} className="px-8 py-3 border border-red-400 text-red-500 rounded-full font-medium hover:bg-red-50 transition w-full sm:w-auto">
                                    Cancel
                    ...
                    {modalState === 'success' && (
                        <div className="bg-white rounded-3xl shadow-2xl p-10 sm:p-14 max-w-lg w-full text-center relative">
                            ...
                            <button onClick={() => window.location.href = '/dashboard'} className="px-8 py-3 bg-[#7C3AED] text-white rounded-full font-medium hover:bg-purple-700 transition w-full max-w-xs shadow-lg">
                                Go To Dashboard
                            </button>
                        </div>
                    )}
